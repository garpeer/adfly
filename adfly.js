...
$:function(){
    ...
    if(arguments.length===1&&(typeof O==="object"||(typeof O==="function"&&!!O.nodeName))){
        ...
        var Q=new G();
    for(var M=0,L,P;(L=arguments[M]);M++){
        if(typeof L==="string"){
            L=L.replace(/^[^#]*(#)/,"$1");
            if(I.id.test(L)){
...
$$:function(Q,N){
    ...
    if(typeof Q==="string"&&P&&P.id!==Q){
        P=null;
        for(var L=0,M;(M=document.all[L]);L++){
            if(M.id===Q){
                P=M;
                break
            }
...
getSequence:function(P){
    var Q,O=2,M=-1,L=-1,N=I.nth.exec(P.replace(/^0n\+/,"").replace(/^2n$/,"even").replace(/^2n+1$/,"odd"));
    if(!N){
        return null
        }
        if(N[2]){
    ...
    for(var Al=0,V;(V=T[Al]);Al++){
        ...
        if(Al>0&&I.relation.test(V)){
            if((d=I.relation.exec(V))){
                ...
                for(var Aj=0,c;(c=At[Aj]);Aj++){
                switch(d[0]){
                    case">":
                        var AF=Ar||AM(Ax,c);
                        for(var Ah=0,AA;(AA=AF[Ah]);Ah++){
                        if(AA.parentNode===c){
                            AI[AI.length]=AA
                            }
                        }
...
if(AH.id){
    var f=0,Ap=document.getElementById(AH.id.replace(/#/,""));
    if(Ap){
...
At.skipTag=false;
if(AH.allClasses){
    var Ac=0,Ae=[],Y=AH.allClasses.split(".").slice(1);
    while((Ak=At[Ac++])){
        var Ai=true,Az=Ak.className;
        if(Az&&Az.length){
            Az=Az.split(" ");
...
if(AH.allAttr){
    var w=0,AE=[],Af=[],AJ=AH.allAttr.match(/\[[^\]]+\]/g);
    for(var z=0,P=AJ.length,x,AQ;z<P;z++){
        ...
        for(var p=0,AG=AE.length;p<AG;p++){
            var R=true,AC=AE[p][0],An=U(Ak,AE[p][1]);
            if(!AC&&An===true){
                continue
            }
...
var d=document.evaluate(f,this,L,0,null),c;
...
makeCall:function(ajaxObj){
    ...
    if(XMLHttp){
        ...
        (function(elm){
            var url=ajaxObj.url,method=ajaxObj.method||"GET",callback=ajaxObj.callback,params=ajaxObj.params,headers=ajaxObj.headers,responseType=ajaxObj.responseType||"text",addToContent=ajaxObj.addToContent,timeout=ajaxObj.timeout||null,ex=ajaxObj.exception,timeoutId=null;
...
handleEvent:function(B){
    ...
    while(H.nodeType!==1&&H.parentNode){
        ...
        G.eventTarget=H;
    var C=this.events[G.type].slice(0),F,E;
    if((F=C.length)){
        for(var D=0;D<F;D++){
...
var Adfly = function(options){
   // try{
        ...
        var url;
        var location = DOMAssistant.$(window).location.href;
        var l = location.length;

        DOMAssistant.$("body a").each(function () {
            var href = DOMAssistant.$(this).href;
            var hostname = DOMAssistant.$(this).hostname;
            if ( href 
